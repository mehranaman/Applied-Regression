...
#import data...
vit <- read.csv("data/data.csv", stringsAsFactors=FALSE) 

names(vit)
#Inital correlationss
vars <- c("D25", "ageyears", "D125", "weight", "bmi", "pth") 
library(psych)
corr.test(vit[,vars])$r
#First model
...
cooksPlot(q_mod2) 
threeOuts(q_mod2)
dfBetas(q_mod2, sort = TRUE)
#Remove the outlier(s)
good_vit <- vit[!row.names(vit) %in% c(182),]
#Final Model
q_mod_f <- lm(D25 ~ ageyears + D125 + bmi + pth, data=good_vit) 
summary(q_mod_f)
lm(D25 ~ D125 + ageyears + bmi + pth, data=good_vit)
